...
public class Light_GUI extends javax.swing.JFrame {
    private Controller controller = null;
    private javax.swing.Timer updateTimer;
    private boolean isSystemOn = false;
    
    public Light_GUI(Controller c) {
        try {
            System.out.println("DEBUG: Starting light GUI initialization");
            ...
            if (controller.getLightSensor() != null) {
                isSystemOn = controller.getLightSensor().isIsRunning();
                updateOnOffButtonState();
            }
            
            updateTimer = new javax.swing.Timer(1000, (e) -> updateLightLabels());
            updateTimer.start();
            
            System.out.println("DEBUG: Setting initial visibility to false");
            this.setVisible(false);
            
            System.out.println("DEBUG: Light GUI initialized successfully");
        } catch (Exception e) {
            System.err.println("DEBUG: Error initializing light GUI: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private void initComponents() {
        ...
        jLabel2.setIcon(new javax.swing.ImageIcon(getClass().getResource("/images/light.png"))); // NOI18N
    ...
    private void on_off_buttonActionPerformed(java.awt.event.ActionEvent evt) {
        try {
            if (controller != null && controller.getLightSensor() != null) {
                LightSensor sensor = controller.getLightSensor();
    ...
    private void updateLightLabels() {
        if (controller != null && controller.getLightSensor() != null) {
            ...
            float currentLight = sensor.getCurrent();
            current.setText(String.format("%.1f", currentLight));
            
            // Update max and min values
            max.setText(String.format("%.1f", sensor.getMaxIntensity()));
            min.setText(String.format("%.1f", sensor.getMinIntensity()));
            
            // Update status based on light level
            if (currentLight > sensor.getMaxIntensity()) {
                status.setText("High");
                status.setForeground(Color.RED);
            } else if (currentLight < sensor.getMinIntensity()) {
                status.setText("Low");
    ...
    public void updateLight(float light) {
        if (controller != null && controller.getLightSensor() != null) {
            LightSensor sensor = controller.getLightSensor();
            current.setText(String.format("%.1f", light));
            ...
            // Update status based on light level
            if (light > sensor.getMaxIntensity()) {
                status.setText("High");
                status.setForeground(Color.RED);
            } else if (light < sensor.getMinIntensity()) {
                status.setText("Low");
                status.setForeground(Color.BLUE);
            } else {
                status.setText("Normal");

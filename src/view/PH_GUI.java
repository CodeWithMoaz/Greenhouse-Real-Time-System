 ...
 * @author Moaz
 */
public class PH_GUI extends javax.swing.JFrame {
    private Controller controller = null;
    private javax.swing.Timer updateTimer;
    private boolean isSystemOn = false;
    
    /**
     * Creates new form PH_GUI
     */
    public PH_GUI(Controller c) {
        try {
            System.out.println("DEBUG: Starting pH GUI initialization");
            ...
            // Initialize system state if sensor exists
            if (controller.getpHSensor() != null) {
                isSystemOn = controller.getpHSensor().isIsRunning();
                updateOnOffButtonState();
            }
            
            // Initialize update timer
            updateTimer = new javax.swing.Timer(1000, (e) -> updatePHLabels());
            updateTimer.start();
            ...
            System.out.println("DEBUG: pH GUI initialized successfully");
        } catch (Exception e) {
            System.err.println("DEBUG: Error initializing pH GUI: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private void initComponents() {
        ...
        jLabel2.setIcon(new javax.swing.ImageIcon(getClass().getResource("/images/ph-up-and-down-1 (Small).jpeg"))); // NOI18N
        ...
        jLabel14.setText("الرقم الهيدروجيني");
    ...
    private void on_off_buttonActionPerformed(java.awt.event.ActionEvent evt) {
        try {
            if (controller != null && controller.getpHSensor() != null) {
                pHSensor sensor = controller.getpHSensor();
    ...
    private void updatePHLabels() {
        if (controller != null && controller.getpHSensor() != null) {
            ...
            float currentPH = sensor.getCurrent();
            current.setText(String.format("%.1f", currentPH));
            
            // Update max and min values
            max.setText(String.format("%.1f", sensor.getMax_pH()));
            min.setText(String.format("%.1f", sensor.getMin_pH()));
            
            // Update status based on pH level
            if (currentPH > sensor.getMax_pH()) {
                status.setText("High");
                status.setForeground(Color.RED);
            } else if (currentPH < sensor.getMin_pH()) {
                status.setText("Low");
    ...
    public void updatePH(float pH) {
        if (controller != null && controller.getpHSensor() != null) {
            pHSensor sensor = controller.getpHSensor();
            current.setText(String.format("%.1f", pH));
            ...
            // Update status based on pH level
            if (pH > sensor.getMax_pH()) {
                status.setText("High");
                status.setForeground(Color.RED);
            } else if (pH < sensor.getMin_pH()) {
                status.setText("Low");
                status.setForeground(Color.BLUE);
            } else {
                status.setText("Normal");
